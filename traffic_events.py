...
#             sel_sxx_scale = sel_sxx
            ...
            for i in range(len(peaks)-1):
                ...
                if (sel_sxx_sum[peaks[i]]>np.quantile(sel_sxx_sum[peaks],pk_pct)) and (ax_t[peaks[i]]-ax_t[peaks[i-1]]>t_h) and (ax_t[peaks[i+1]]-ax_t[peaks[i]]>t_h):
                    loc_peaks.append(peaks[i])#+nframe*sel_sxx_sum.shape)


        dA_1=np.zeros((len(loc_peaks),len(Sxx)))
        dA_2=np.zeros((len(loc_peaks),len(Sxx)))
        for i in range(len(loc_peaks)-1):
            dA_1[i,:]=np.log(Sxx[:,loc_peaks[i]-c0])-np.log(Sxx[:,loc_peaks[i]-c0-d_ind])
            dA_2[i,:]=np.log(Sxx[:,loc_peaks[i]+c0])-np.log(Sxx[:,loc_peaks[i]+c0+d_ind])
        
        
        dA_a=np.vstack((dA_1,dA_2))
        np.save('ax_f', ax_f)
        
        
        title = self.gettitle(flag='traffic', starttime=win_bgn, endtime=win_end, cmp=self.component)
        path = self.par['data_output_path'] + title
        self.writedata(data=dA_a, ax1=np.array([]), 
                       time=win_bgn.strftime("%Y-%m-%d"), file_path=path, txt_hrd=title)
